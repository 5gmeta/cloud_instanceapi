...
#        payload = Instance.from_dict(connexion.request.get_json())  # noqa: E501
    ...
    url = 'http://discovery-api.vicomtech.svc.cluster.local:8080/discovery-api/mec/' + str(mec_id) + '/nbservices'
    ...
    try:
        url = 'http://' + edgeinstance_ip + ':' + str(edgeinstance_port) + '/instances'
...
def get_instance(mec_id, instance_id):  # noqa: E501
    ...
    edgeinstance_port = json_response[service_index]["port"]
    try:
        url = 'http://' + edgeinstance_ip + ':' + str(edgeinstance_port) + '/instances/' + str(instance_id)
        headers = {"accept": "application/json"}
        response = requests.get(url, headers=headers, verify=False)
        json_response = json.loads(response.content)
    except:
